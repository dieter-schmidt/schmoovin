...
{
    class MeleeWeaponNewAnimatorStep : NeoFpsWizardStep
    {
        [Tooltip("The looping idle animation.")]
        public AnimationClip idleAnimation = null;

        [Tooltip("The attack animation clip.")]
        public AnimationClip attackAnimation = null;
        [Tooltip("The attack hit animation clip. Played in response to striking a solid object.")]
        public AnimationClip attackHitAnimation = null;
        ...
        public override void CheckStartingState(NeoFpsWizard wizard)
        {
            ...
            if (attackAnimation == null || idleAnimation == null)
                m_CanContinue = false;
            var viewModel = wizard.steps[MeleeWeaponWizardSteps.viewModel] as MeleeWeaponViewModelStep;
            if (viewModel.sprintAnimations == 2 && sprintAnimation == null)
                m_CanContinue = false;
        }
        ...
        void OnValidate()
        {
            ...
            sprintClipSpeed = Mathf.Clamp(sprintClipSpeed, 1f, 50f);
        }
        ...
        protected override void OnInspectorGUI(SerializedObject serializedObject, NeoFpsWizard wizard)
        {
            ...
            m_CanContinue &= NeoFpsEditorGUI.RequiredObjectField(serializedObject.FindProperty("idleAnimation"));

            m_CanContinue &= NeoFpsEditorGUI.RequiredObjectField(serializedObject.FindProperty("attackAnimation"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("attackHitAnimation"));
        ...
        protected override void OnSummaryGUI(NeoFpsWizard wizard)
        {
            ...
            WizardGUI.ObjectSummary("Attack Animation", attackAnimation);
            WizardGUI.ObjectSummary("Attack Hit Animation", attackHitAnimation);
